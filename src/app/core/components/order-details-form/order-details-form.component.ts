...
export class OrderDetailsFormComponent implements OnInit, OnDestroy {
  ...
  commentInput = new FormControl('', [Validators.maxLength(250)]);
  ...
  changeOrderDetails(): void {
    if (this.isPopup)
      this.closeEmmiter.emit(
        'Ваш заказ успешно изменен, ожидайте доставку в указанное время'
      );
    if (this.orderId) {
      this.subscription.add(
        this.dataService
          .changeOrderDetais(this.orderId, this.getDetails())
          .subscribe((res) => {
            if (res) {
              this.store.dispatch(getUserChanges());
            }
