...
import { concatAll, map, mergeAll, mergeMap, switchMap, take } from 'rxjs/operators';
...
export class FavotirePageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription = this.store.select((state) => state.user.user?.favorites).subscribe(ids => {
      if (ids) {
